...
YOU MAY MODIFY THE LINE net = Network    ().to(device)
"""
...
class NetworkCnn(tnn.Module):
    ...
    maximum val from the entire output channel. See Kim ethttps://pytorch.org/docs/stable/nn.html#torch.nn.utils.rnn.PackedSequence. al. 2014:
    ...
    def forward(self, input, length):
        ...
        Create the forward pass through the network.
        """
        out = self.maxpool(self.relu(self.conv1(input.permute(0,2,1))))
        out = self.dropout(out)
        out = self.maxpool(self.relu(self.conv2(out)))
        out = self.dropout(out)
        out = self.globalmaxpool(self.relu(self.conv3(out)))
...
def measures(outputs, labels):
    ...
    confusion_mat=outputs/labels
    tp = torch.sum(confusion_mat==1).item()
    fp = torch.sum(confusion_mat==float('inf')).item()
    tn = torch.sum(torch.isnan(confusion_mat)).item()
    fn = torch.sum(confusion_mat==0).item()
    return tp,tn,fp,fn
