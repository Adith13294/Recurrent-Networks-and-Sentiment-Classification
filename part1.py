...
class rnn(torch.nn.Module):
    ...
    def rnnCell(self, input, hidden):
        ...
        """
        tan=torch.nn.Tanh()
        add=self.ih(input).add(self.hh(hidden))
    ...
    def forward(self, input):
        ...
        for inp in range(input.size(0)):
            hidden=self.rnnCell(input[inp],hidden)
...
def lstm(input, hiddenSize):
    ...
    lstm = torch.nn.LSTM(input.size(2), hiddenSize, bias=True, batch_first=True)
...
def conv(input, weight):
    ...
    """
    out=torch.nn.functional.conv1d(input,weight)
    return out
